...
public class ParticleSet {
	...
	public ParticleSet(ParticleSet set) {
		this.numParticles = set.numParticles;
		particles = new Particle[numParticles];

		for (int i = 0; i < numParticles; i++) {
			particles[i] = new Particle(new Pose(
					set.getParticle(i).getPose().getX(),
					set.getParticle(i).getPose().getY(),
					set.getParticle(i).getPose().getHeading()));
		}
	}
